...
{
    // Convert Airy:// to Airy:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Airy://"))
    {
        uri.replace(0, 12, "Airy:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Airy.lnk";
}
...
{
    return GetAutostartDir() / "Airy.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Airy-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Airy-qt [" + tr("command-line options") + "]                     " + "\n";
